...
import {
  ...
  RefreshCw, ExternalLink, X, Terminal, Zap, Bitcoin,
} from "lucide-react";
import {
...
const FAQ_ITEMS = [
  ...
  { q: "Can I pay with cryptocurrency?", a: "Yes! Click 'Pay with Crypto' on any plan to pay with Bitcoin, Ethereum, USDT, and 300+ other cryptocurrencies via NOWPayments. Crypto subscriptions renew via email invoice each period." },
];

// ─── Types ────────────────────────────────────────────────────────────────────
type FeatureRowDef = { label: string; hint?: string; render: (plan: typeof PLANS[number]) => React.ReactNode; };
    ...
    <InfoPopover label="Pro (forever)">
      <ul className="space-y-1 mb-2">
        {["Permanent email retention", "Fresh rotated domain list", "Private inboxes"].map(i => (
          <li key={i} className="flex items-start gap-1.5 text-xs text-foreground"><Check className="h-3 w-3 mt-0.5 shrink-0" />{i}</li>
        ))}
      </ul>
...
function SlaBadge() {
  return (
    <InfoPopover label="99.5% SLA">
      ...
      <a href="https://status.freecustom.email" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
...
function CliBadge({ withWatch }: { withWatch: boolean }) {
  if (!withWatch) return <Link href="/api/cli" className="text-xs font-medium text-foreground underline underline-offset-4 decoration-border hover:decoration-foreground transition-colors">CLI</Link>;
  return (
...
// ─── Billing preview ──────────────────────────────────────────────────────────
const PLAN_PRICES: Record<string, number> = { developer: 7, startup: 19, growth: 49, enterprise: 149 };

function calcUpgradePreview(fromPlan: string, toPlan: string, nextBilledAtStr: string | null, billing: "monthly" | "yearly" = "monthly") {
  let fromPrice = PLAN_PRICES[fromPlan.toLowerCase()] ?? 0;
  let toPrice   = PLAN_PRICES[toPlan.toLowerCase()]   ?? 0;
  const now = new Date();
  ...
  if (isCurrentYearly && YEARLY_PRICING[fromPlan.toLowerCase()]) fromPrice = parseInt(YEARLY_PRICING[fromPlan.toLowerCase()].total.replace(/[^0-9]/g, ""), 10);
  if (billing === "yearly"     && YEARLY_PRICING[toPlan.toLowerCase()])   toPrice   = parseInt(YEARLY_PRICING[toPlan.toLowerCase()].total.replace(/[^0-9]/g, ""),   10);
  ...
  if (nextBilledAt) {
    ...
    if (isCurrentYearly) lastBilledAt.setFullYear(lastBilledAt.getFullYear() - 1);
    else                 lastBilledAt.setMonth(lastBilledAt.getMonth() - 1);
  ...
  return { proratedToday: Math.max(0, parseFloat((diff * fraction).toFixed(2))), nextMonthly: toPrice, nextBilledAt, daysRemaining, totalDays };
}

// ─── Upgrade modal (Paddle) ───────────────────────────────────────────────────
interface UpgradeModalProps { open: boolean; fromPlan: string; toPlan: string; billing: "monthly" | "yearly"; nextBilledAt: string | null; onConfirm: () => Promise<void>; onClose: () => void; }
function UpgradeModal({ open, fromPlan, toPlan, billing, nextBilledAt, onConfirm, onClose }: UpgradeModalProps) {
  ...
  const fmtDate = (d: Date | null) => d ? d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "next billing date";
  const fmtUSD  = (n: number) => n.toLocaleString("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 2 });
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Zap className="h-4 w-4 text-amber-500" />Upgrade to {toPlan}</DialogTitle>
          <DialogDescription>Upgrading from <strong>{fromPlan}</strong> to <strong>{toPlan}</strong>. Features unlock immediately.</DialogDescription>
        </DialogHeader>
          ...
          <div className="px-4 py-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="font-mono text-[10px] text-muted-foreground mt-0.5">Normal {billing} billing</p>
              </div>
              <span className="text-lg font-bold text-foreground tabular-nums">{fmtUSD(preview.nextMonthly)}<span className="text-xs font-normal text-muted-foreground">/{billing === "yearly" ? "yr" : "mo"}</span></span>
            </div>
          </div>
...
// ─── Downgrade modal ──────────────────────────────────────────────────────────
interface DowngradeModalProps { open: boolean; fromPlan: string; toPlan: string; onConfirm: (reason: string, comment: string) => Promise<void>; onClose: () => void; }
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-1.5">
          ...
          {DOWNGRADE_REASONS.map(r => (
            <button key={r.value} onClick={() => setReason(r.value)}
              className={cn("w-full text-left text-sm px-3 py-2 rounded-md border transition-colors",
        ...
        <DialogFooter className="gap-2">
          ...
          <Button variant="destructive" disabled={busy || !reason}
            onClick={async () => { if (!reason) { toast.error("Please select a reason."); return; } setBusy(true); try { await onConfirm(reason, comment); } finally { setBusy(false); } }}>
...
// ─── AutoBilling modal (Paddle saved method) ──────────────────────────────────
interface AutoBillingModalProps { open: boolean; type: "plan" | "credits"; productName: string; priceLabel: string; isYearly: boolean; onConfirm: () => Promise<void>; onClose: () => void; onOpenPaddleCheckout: () => void; }
function AutoBillingModal({ open, type, productName, priceLabel, isYearly, onConfirm, onClose, onOpenPaddleCheckout }: AutoBillingModalProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/paddle/portal-session", { method: "POST" }).then(r => r.json()).then(d => { if (d.url) setPortalUrl(d.url); }).catch(() => {});
  }, [open]);
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Your saved payment method will be charged for the <strong>{productName}</strong>.</DialogDescription>
        </DialogHeader>
        <div className="rounded-lg border border-border overflow-hidden text-sm">
          <div className="px-4 py-3 flex items-center justify-between">
            <div>
              <p className="font-medium text-foreground">{productName}</p>
              <p className="font-mono text-[10px] text-muted-foreground mt-0.5">{isYearly ? "Billed annually" : "Billed monthly"}</p>
            </div>
            <span className="text-lg font-bold text-foreground tabular-nums">{priceLabel}</span>
          </div>
        </div>
        ...
        <div className="text-xs text-muted-foreground px-1">
          ...
          {portalUrl && <a href={portalUrl} target="_blank" rel="noopener noreferrer" className="ml-1 underline underline-offset-2 hover:text-foreground">Update payment method →</a>}
        </div>
        <DialogFooter className="gap-2">
          <Button variant="outline" onClick={onClose} disabled={busy}>Cancel</Button>
          <Button onClick={async () => { setBusy(true); setError(null); try { await onConfirm(); } catch (e: any) { setError(e.message); if (e.message.includes("Manual checkout required")) { onOpenPaddleCheckout(); onClose(); } } finally { setBusy(false); } }} disabled={busy}>
...
// ─── Plan CTA — Paddle primary + Crypto secondary ────────────────────────────
interface PlanCtaProps {
  ...
  // Crypto sub state (from api-status)
  hasCryptoSub:          boolean;
  currentCryptoPlan:     string | null;
  currentCryptoSubId?:   string | null;
  onUpgrade:             (p: typeof PLANS[number]) => void;
  onDowngrade:           (p: typeof PLANS[number]) => void;
  onGetStarted:          (p: typeof PLANS[number]) => void;
}
function PlanCta({ plan, currentPlan, isLoggedIn, busy, billing, scheduledDowngradePlan, hasCryptoSub, currentCryptoPlan, onUpgrade, onDowngrade, onGetStarted }: PlanCtaProps) {
  ...
  if (planName === currentPlan) {
    return <Button size="sm" variant="outline" className="w-full cursor-default" disabled>{scheduledDowngradePlan ? "Changing soon" : "Current plan"}</Button>;
  }
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      <Button size="sm" variant="outline" className="w-full text-muted-foreground hover:text-foreground" disabled={busy} onClick={() => onDowngrade(plan)}>
        {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Downgrade"}
      </Button>
      <CryptoCheckoutButton
...
interface MobilePlanCardProps {
  plan: typeof PLANS[number]; billing: "monthly" | "yearly";
  currentPlan: ApiPlanName | null; isLoggedIn: boolean; busy: boolean;
  scheduledDowngradePlan: string | null; featureGroups: FeatureGroup[];
  hasCryptoSub: boolean; currentCryptoPlan: string | null;
  onUpgrade: (p: typeof PLANS[number]) => void;
  onDowngrade: (p: typeof PLANS[number]) => void;
  onGetStarted: (p: typeof PLANS[number]) => void;
}
function MobilePlanCard({ plan, billing, currentPlan, isLoggedIn, busy, scheduledDowngradePlan, featureGroups, hasCryptoSub, currentCryptoPlan, onUpgrade, onDowngrade, onGetStarted }: MobilePlanCardProps) {
  ...
  return (
    <div className={cn("rounded-lg border overflow-hidden", isPopular && !isCurrent ? "border-foreground/30 ring-2 ring-foreground/20" : "border-border")}>
      <div className={cn("relative px-5 py-5", isPopular && !isCurrent ? "bg-muted/10" : "bg-background", isCurrent && "bg-muted/20")}>
        {isPopular && !isCurrent && <span className="absolute top-0 right-0 font-mono text-[8px] uppercase tracking-widest bg-foreground text-background px-2 py-1 rounded-bl-lg">🔥 Popular</span>}
        {isCurrent && <span className="absolute top-0 right-0 font-mono text-[8px] uppercase tracking-widest bg-muted/60 text-foreground px-2 py-1 rounded-bl-lg">{scheduledDowngradePlan ? "Changing soon" : "Current"}</span>}
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            ) : <div className="flex items-baseline gap-0.5"><span className="text-3xl font-bold text-foreground">{plan.price}</span></div>}
          </div>
          <div className="shrink-0 pt-5 w-28">
            <PlanCta plan={plan} currentPlan={currentPlan} isLoggedIn={isLoggedIn} busy={busy}
              billing={billing} scheduledDowngradePlan={scheduledDowngradePlan}
              hasCryptoSub={hasCryptoSub} currentCryptoPlan={currentCryptoPlan}
      ...
      <div className="border-t border-border divide-y divide-border/60">
        {featureGroups.map(group => (
          <div key={group.group}>
            <div className="px-5 py-2 bg-muted/20"><span className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">{group.group}</span></div>
            {group.rows.map(row => (
              <div key={row.label} className="flex items-center justify-between gap-4 px-5 py-2.5">
                ...
                <div className="shrink-0 text-right">{row.render(plan)}</div>
              </div>
            ))}
          </div>
        ))}
...
export default function ApiPricingPage() {
  ...
  const [hasPaddleSub,         setHasPaddleSub]     = useState(false);
  const [hasCryptoSub,         setHasCryptoSub]     = useState(false);
  const [currentCryptoPlan,    setCurrentCryptoPlan] = useState<string | null>(null);
  ...
  // ── Single fetch for api-status (was duplicated before) ──────────────────
  useEffect(() => {
    ...
    fetch("/api/user/api-status")
      ...
      .then(d => {
        ...
        // Next billing date (from active subscription, either provider)
        setNextBilledAt(data?.subscription?.next_billed_at ?? null);

        // Paddle subscription presence
        setHasPaddleSub(!!(data?.paddle_subscription?.subscription_id));

        // NOWPayments subscription presence + what plan it's on
        const cryptoSub = data?.crypto_subscription;
        const cryptoActive = cryptoSub?.status === "ACTIVE" || cryptoSub?.status === "PENDING_RENEWAL";
        setHasCryptoSub(!!cryptoSub?.subscription_id && cryptoActive);
        setCurrentCryptoPlan(cryptoActive ? (name ?? null) : null);
      })
  ...
  const handleUpgrade = (plan: typeof PLANS[number]) => {
    ...
    // If user has a Paddle sub → show prorated upgrade modal
    if (hasPaddleSub && currentPlan && currentPlan !== "free") { setUpgradeTarget(plan); return; }
    // If user has a crypto sub → CryptoCheckoutButton handles it inline
    // If no sub at all → auto-charge attempt (Paddle saved method)
  ...
  const handleCredits      = (pkg: typeof CREDITS[number]) => {
    ...
    setAutoBillingTarget({ type: "credits", id: pkg.package, name: `${pkg.requests} Credits`, priceLabel: pkg.amount, isYearly: false });
  };
  ...
  const attemptAutoCharge  = async () => {
    ...
    const body = autoBillingTarget.type === "plan"
      ...
      : { priceId: autoBillingTarget.id };
    const res  = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    const data = await res.json();
    if (!res.ok || data.error) throw new Error(data.message || "Auto-charge failed. Please proceed with manual checkout.");
  ...
  const FEATURE_GROUPS: FeatureGroup[] = [
    { group: "Core Features", rows: [
      { label: "Real-time Email", hint: "Wait API (long polling)", render: p => p.wait ? (typeof p.wait === "string" ? <span className="text-xs font-medium text-foreground">{p.wait}</span> : <Tick />) : <Cross /> },
      { label: "Webhooks",       hint: "Real-time HTTP push notifications", render: p => p.webhooks ? <Tick /> : <Cross /> },
      { label: "MCP Access",     hint: "Model Context Protocol for AI agents/LLMs", render: p => p.mcp ? (typeof p.mcp === "string" ? <span className="text-xs font-medium text-foreground">{p.mcp}</span> : <Tick />) : <Cross /> },
      { label: "OTP Extraction", hint: "Auto-parse one-time codes from emails", render: p => p.otp ? <Tick /> : <Cross /> },
      { label: "IMAP Access",    render: p => p.imap ? <span className="font-mono text-[9px] uppercase tracking-widest text-emerald-600/90 border border-emerald-600/30 rounded px-1.5 py-px">Soon</span> : <Cross /> },
      { label: "Custom Domains", hint: "Register inboxes on your own domain", render: p => p.customDomains ? <Tick /> : <Cross /> },
      { label: "Attachments",    render: p => p.attachments ? <span className="text-xs font-medium text-foreground">{p.maxAttachment}</span> : <Cross /> },
      { label: "Uptime SLA",     render: p => p.sla ? <SlaBadge /> : <Cross /> },
    ]},
    { group: "Deliverability", rows: [
    ...
    { group: "Rate Limits", rows: [
      ...
      { label: "Requests / sec",    render: p => <span className="text-sm font-semibold text-foreground tabular-nums">{p.reqSec}</span> },
      { label: "Requests / month",  render: p => <span className="text-xs font-semibold text-foreground tabular-nums">{p.reqMonth}</span> },
      { label: "WebSocket conns",   render: p => p.ws ? <span className="text-xs font-medium text-foreground">{p.maxWs}</span> : <Cross /> },
    ]},
    { group: "Observability / Debugger", rows: [
      { label: "Timeline History",  hint: "How long test runs are retained in the dashboard", render: p => <span className="text-xs font-semibold text-foreground">{p.timeline}</span> },
      { label: "Live WebSocket",    hint: "Real-time UI timeline updates", render: p => p.live ? <Check/> : <Cross /> },
      { label: "Failure Insights",  hint: "Automated test flow issue detection", render: p => p.insights ? <Check /> : <Cross /> },
    ]},
    { group: "Developer", rows: [
      { label: "CLI",     hint: "fce CLI works on all plans", render: p => <CliBadge withWatch={!!p.ws} /> },
      { label: "Support", render: p => { const map: Record<string, string> = { free: "Community", developer: "Email", startup: "Email", growth: "Priority", enterprise: "Dedicated" }; const val = map[p.name] ?? "Email"; return <span className={cn("text-xs", val === "Dedicated" ? "text-foreground font-medium" : val === "Priority" ? "text-foreground" : "text-muted-foreground")}>{val}</span>; } },
    ]},
  ];
  ...
  return (
    <>
      <div className="bg-background text-foreground" style={DOT_BG}>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-16">
          ...
          <div className="mb-12">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-foreground mb-2">Simple, predictable pricing</h1>
            ...
            <div className="inline-flex items-center rounded-lg border border-border bg-muted/20 p-1 gap-1">
              <button onClick={() => setBilling("monthly")} className={cn("relative px-4 py-1.5 rounded-md text-sm font-medium transition-all duration-200", billing === "monthly" ? "bg-background text-foreground shadow-sm border border-border" : "text-muted-foreground hover:text-foreground")}>Monthly</button>
              <button onClick={() => setBilling("yearly")}  className={cn("relative flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all duration-200", billing === "yearly" ? "bg-background text-foreground shadow-sm border border-border" : "text-muted-foreground hover:text-foreground")}>
                Yearly
                <span className={cn("font-mono text-[9px] uppercase tracking-widest rounded px-1.5 py-px transition-colors", billing === "yearly" ? "bg-emerald-600/15 text-emerald-600 border border-emerald-600/25" : "bg-muted/40 text-muted-foreground border border-border")}>2 months free</span>
              </button>
            </div>
            ...
            {scheduledDowngradePlan && (
              <div className="mt-5 flex items-start gap-3 rounded-lg border border-border bg-muted/20 px-4 py-3 text-sm">
                ...
                <p className="text-muted-foreground">Scheduled change to <strong className="text-foreground capitalize">{scheduledDowngradePlan}</strong> at end of billing period.</p>
              </div>
            )}

            {/* Crypto sub notice */}
            {hasCryptoSub && (
              <div className="mt-3 flex items-center gap-2 rounded-lg border border-orange-500/20 bg-orange-500/5 px-4 py-2.5 text-sm w-fit">
                <Bitcoin className="h-3.5 w-3.5 text-orange-500 shrink-0" />
                <p className="text-muted-foreground text-xs">You have an active crypto subscription. To change plans, use the <strong className="text-foreground">Upgrade / Downgrade with Crypto</strong> button on each plan.</p>
              </div>
            )}
          </div>
              ...
              <div className="min-w-[700px]">
                <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm border-b border-border">
                  <div className="grid grid-cols-[200px_repeat(5,1fr)] gap-px bg-border rounded-t-lg overflow-hidden">
                    <div className="bg-background px-4 py-5 flex items-end"><span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Plan</span></div>
                    {PLANS.map(plan => {
                      const isPopular  = plan.name === "startup";
                      const isCurrent  = isLoggedIn && currentPlan === plan.name;
                      ...
                      return (
                        <div key={plan.name} className={cn(getColClass(plan.name), "px-4 py-5 relative")}>
                          {isPopular && !isCurrent && <span className="absolute top-0 right-0 font-mono text-[8px] uppercase tracking-widest bg-foreground text-background px-2 py-1 rounded-bl-lg">🔥 Popular</span>}
                          {isCurrent && <span className="absolute top-0 right-0 font-mono text-[8px] uppercase tracking-widest bg-muted/60 text-foreground px-2 py-1 rounded-bl-lg">{isScheduled ? "Scheduled" : "Current"}</span>}
                          ...
                          {billing === "monthly" || plan.name === "free" ? (
                            <>
                              ...
                              {plan.price !== "$0" ? <p className="font-mono text-[9px] text-muted-foreground/50 mb-3">billed monthly</p> : <div className="mb-[1.125rem]" />}
                            </>
                          ) : (
                            <>
                              <div className="flex items-baseline gap-0.5">
                                <span className="text-2xl font-bold text-foreground">{YEARLY_PRICING[plan.planId!]?.effective ?? plan.price}</span>
                                <span className="text-xs text-muted-foreground">/mo</span>
                              </div>
                              <p className="font-mono text-[9px] text-emerald-600/90 mb-3">{YEARLY_PRICING[plan.planId!]?.total}/year · 2 months free</p>
                            </>
                          )}
                          {planLoading && isLoggedIn
                            ...
                            : <PlanCta plan={plan} currentPlan={currentPlan} isLoggedIn={isLoggedIn}
                                busy={busyPlanId === plan.planId} billing={billing}
                  ...
                  {FEATURE_GROUPS.map(group => (
                    <div key={group.group}>
                      <div className="grid grid-cols-[200px_repeat(5,1fr)] bg-muted/30">
                        <div className="px-4 py-2.5 col-span-6"><span className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">{group.group}</span></div>
                      </div>
                      {group.rows.map(row => (
                        <div key={row.label} className="grid grid-cols-[200px_repeat(5,1fr)] gap-px bg-border hover:bg-border/80 transition-colors">
                          ...
                          {PLANS.map(plan => (
                            <div key={plan.name} className={cn(getColClass(plan.name), "px-4 py-3 flex items-center justify-center")}>{row.render(plan)}</div>
                          ))}
                        </div>
                      ))}
                    </div>
          ...
          <div className="lg:hidden space-y-4">
            <p className="text-[11px] text-muted-foreground font-mono">Tap a plan to see all features. Scroll to compare.</p>
            {PLANS.map(plan => (
              <MobilePlanCard key={plan.name} plan={plan} billing={billing}
                currentPlan={currentPlan} isLoggedIn={isLoggedIn} busy={busyPlanId === plan.planId}
                scheduledDowngradePlan={scheduledDowngradePlan} featureGroups={FEATURE_GROUPS}
                hasCryptoSub={hasCryptoSub} currentCryptoPlan={currentCryptoPlan}
                onUpgrade={handleUpgrade} onDowngrade={handleDowngrade} onGetStarted={handleGetStarted} />
            ))}
          </div>
          ...
          <div className="mt-4 space-y-2">
            <div className="flex items-start gap-3 rounded-lg border border-border bg-muted/10 px-4 py-3">
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed"><span className="text-foreground font-semibold">Growth &amp; Enterprise:</span>{" "}Inboxes include permanent retention, fresh domain rotation, and private access at the backend level. Client-side features (ad-free UI, OTP chips) require a separate{" "}<Link href="/pricing" className="underline underline-offset-4 decoration-border hover:text-foreground transition-colors">webapp Pro plan →</Link></p>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-border bg-muted/10 px-4 py-3">
              <Terminal className="h-4 w-4 text-muted-foreground mt-0.5 shrink-0" />
              <p className="text-[11px] text-muted-foreground leading-relaxed"><span className="text-foreground font-semibold">CLI on all plans.</span>{" "}Live watching (<code className="bg-muted/40 px-1 rounded text-foreground text-[10px]">fce watch</code>, <code className="bg-muted/40 px-1 rounded text-foreground text-[10px]">fce dev</code>) requires Startup+. OTP extract (<code className="bg-muted/40 px-1 rounded text-foreground text-[10px]">fce otp</code>) requires Growth+.<Link href="/api/cli" className="ml-1 underline underline-offset-4 decoration-border hover:text-foreground transition-colors">CLI docs →</Link></p>
            </div>
          </div>

          {/* Trust strip */}
          <div className="flex flex-wrap items-center gap-3 mt-6 pt-6 border-t border-border">
            <span className="flex items-center gap-1.5 border border-border rounded-md px-3 py-1.5 bg-muted/20 text-xs text-muted-foreground font-mono whitespace-nowrap"><ShieldCheck className="h-3.5 w-3.5 text-foreground/70 shrink-0" />Secure checkout via Paddle</span>
            <div className="flex items-center gap-1">
              {PAYMENT_METHODS.map(({ icon: Icon, label }) => (
                <span key={label} title={label} className="flex items-center justify-center rounded border border-border bg-muted/20 px-2 py-1.5 text-muted-foreground/70 hover:text-muted-foreground transition-colors"><Icon className="h-3.5 w-auto" /></span>
              ))}
              {/* Crypto accepted badge */}
              <span title="Crypto accepted" className="flex items-center justify-center rounded border border-orange-500/30 bg-orange-500/5 px-2 py-1.5 text-orange-500/70 hover:text-orange-500 transition-colors">
                <Bitcoin className="h-3.5 w-3.5" />
              </span>
            </div>
            <span className="flex items-center gap-1 text-[10px] font-mono text-muted-foreground border border-border rounded-md px-2.5 py-1.5 bg-muted/20"><Globe className="h-3 w-3 shrink-0" />200+ countries</span>
          </div>
          ...
          <div className="mt-16">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground max-w-sm leading-relaxed">Credits never expire. Consumed automatically when you exceed your monthly quota.</p>
              </div>
            </div>
            ...
            <div className="grid gap-px bg-border rounded-lg overflow-hidden grid-cols-2 sm:grid-cols-4">
              {CREDITS.map(c => (
                <div key={c.package} className="bg-background px-5 py-6 flex flex-col gap-3">
                  ...
                  {/* Paddle: auto-charge saved method */}
                  <Button size="sm" variant="outline" className="w-full" disabled={!!busyCredits} onClick={() => handleCredits(c)}>
                    {busyCredits === c.package ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Buy credits"}
                  </Button>
                  {/* Crypto: always available */}
                  <CryptoCheckoutButton type="credits" package={c.package} className="w-full" />
                </div>
              ))}
            </div>

            <div className="mt-4 grid gap-px bg-border rounded-lg overflow-hidden grid-cols-2 sm:grid-cols-4">
              {[{ v: "Never expire", l: "use whenever you need" }, { v: "Stackable", l: "with any plan" }, { v: "Auto-consumed", l: "after monthly quota" }, { v: "Any time", l: "buy more anytime" }].map(({ v, l }) => (
      ...
      <UpgradeModal open={upgradeTarget != null} fromPlan={PLANS.find(p => p.name === currentPlan)?.label ?? ""} toPlan={upgradeTarget?.label ?? ""} billing={billing} nextBilledAt={nextBilledAt} onConfirm={confirmUpgrade} onClose={() => { setUpgradeTarget(null); setBusyPlanId(null); }} />
      <DowngradeModal open={downgradeTarget != null} fromPlan={PLANS.find(p => p.name === currentPlan)?.label ?? ""} toPlan={downgradeTarget?.label ?? ""} onConfirm={confirmDowngrade} onClose={() => { setDowngradeTarget(null); setBusyPlanId(null); }} />
      <AutoBillingModal open={autoBillingTarget !== null} type={autoBillingTarget?.type!} productName={autoBillingTarget?.name!} priceLabel={autoBillingTarget?.priceLabel!} isYearly={autoBillingTarget?.isYearly!} onConfirm={attemptAutoCharge} onClose={() => { setAutoBillingTarget(null); setBusyPlanId(null); setBusyCredits(null); }}
        onOpenPaddleCheckout={() => {
          if (autoBillingTarget?.type === "plan") openCheckout({ productType: "api", apiPlan: autoBillingTarget.id, billing: autoBillingTarget.isYearly ? "yearly" : "monthly" });
          else openCheckout({ productType: "credits", package: autoBillingTarget!.id });
        }} />
      <PaddleInit />
    </>
  );
