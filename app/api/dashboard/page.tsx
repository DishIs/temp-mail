...
import {
  CreditCard, Key, Inbox, Check, X, Loader2, Copy,
  Trash2, Zap, BarChart3, Receipt, AlertCircle,
} from "lucide-react";
      ...
      <div className="min-h-screen bg-background" style={DOT_BG}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 py-10">
          ...
          <Tabs defaultValue="overview" className="w-full">
            {/* Tab strip — underline style like Firecrawl */}
            <TabsList className="h-auto w-full justify-start bg-transparent border-0 border-b border-border rounded-none gap-0 p-0 mb-8">
              {["overview", "keys", "usage", "billing"].map(tab => (
                <TabsTrigger key={tab} value={tab}
                  className="rounded-none px-4 pb-3 pt-0 text-sm border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:text-foreground data-[state=inactive]:text-muted-foreground data-[state=active]:shadow-none capitalize">
                ...
                <TabsContent value="overview" className="mt-0">
                  ...
                  <div className="grid gap-px bg-border sm:grid-cols-3 rounded-lg overflow-hidden mb-8">
                    <div className="bg-background px-6 py-6">
                      <StatTile label="Requests this month"
                    ...
                    <div className="bg-background px-6 py-6">
                      <StatTile label="Credits balance"
                        value={Number(credits).toLocaleString()}
                        sub="Never expire · used after quota" />
                    </div>
                    <div className="bg-background px-6 py-6">
                      <StatTile label="Rate limit"
                        value={`${rateLimitSec} req/s`}
